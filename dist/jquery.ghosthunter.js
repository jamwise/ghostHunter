...
(function(root, factory){
	...
	$.fn.ghostHunter.defaults = {
		...
		onPageLoad			: true,
		onKeyUp				: false,
		result_template 	: "<a id='new-gh-{{ref}}' class='gh-search-item' href='{{link}}'><p><h2>{{title}}</h2><h4>{{prettyPubDate}}</h4></p></a>",
	...
	var grabAndIndex = function(){
		...
		$.get(ghost.url.api('posts',params)).done(function(data){
			...
			me.index = lunr(function () {
				this.ref('id');
				this.field('title', {boost: 10, usePipeline: false, wildcard: lunr.Query.wildcard.TRAILING});
				this.field('description', {boost: 5, usePipeline: false, wildcard: lunr.Query.wildcard.TRAILING});
				this.field('plaintext', {usePipeline: false, wildcard: lunr.Query.wildcard.TRAILING});
				this.field('pubDate');
				this.field('tag');
	...
	var pluginMethods	= {
		...
		init			: function( target , opts ){
			...
			// For unknown reasons, some DOM changes during the first attempt to search
			// in a page did not show up in the displayed page. Triggering an empty
			// search on instantiation worked around it.
			this.find("");
			...
			lastTimeoutID = setTimeout(function() {
				...
				for (var i=0,ilen=valueSplit.length;i<ilen;i++) {
					...
					thingsFound.push(this.index.query(function (q) {
						...
						q.term(v, {boost: 100, usePipeline: true});
						// look for terms that match the beginning of this queryTerm and apply a medium boost
						q.term(v, {boost: 10, usePipeline: false, wildcard: lunr.Query.wildcard.TRAILING});
						// look for terms that match with an edit distance of 1 and apply a small boost
						q.term(v, { usePipeline: false, editDistance: 1, boost: 1 });
					}));
				}
				var searchResult;
				if (thingsFound.length > 1) {
					// If we had multiple terms, we'll have multiple lists.
					// Find the shortest one, and filter out any entries not found in
					// all of the other lists.
					// This spoofs an AND join between terms, which lunr.js can't yet do internally.
					var shortestIndex = 0;
					var shortestLength = null;
					for (var i=0,ilen=thingsFound.length;i<ilen;i++) {
						if (shortestLength === null || thingsFound[i].length < shortestLength) {
							shortestLength = thingsFound[i].length;
							shortestIndex = i;
						}
					}
					var shortestList = thingsFound[shortestIndex];
					thingsFound = thingsFound.slice(0, i).concat(thingsFound.slice(i+1));
					for (var i=shortestList.length-1;i>-1;i--) {
						var ref = shortestList[i].ref;
						for (j=0,jlen=thingsFound.length;j<jlen;j++) {
							...
							if (!otherRefs[ref]) {
								shortestList = shortestList.slice(0, i).concat(shortestList.slice(i+1));
								break;
							}
						}
					}
					searchResult = shortestList;
